...
class CLIP(nn.Module):
    def __init__(self, args, task_classnames, clip_model, task_info, n_ctx=12):
        ...
        with torch.no_grad():
            ...
            entity_embeddings = self.text_encoder(tokenized_keys,getEmbedding=True)
            entity_embeddings /= entity_embeddings.norm(dim=-1, keepdim=True)
            self.entity_keys = entity_embeddings #(260,768)
            # todo 后面要测试将entity作为key和value两种情况的效果
    ...
    def forward(self, image, num_test=None, test_class=None, test=False):
        ...
        if not test:
            # 3.1 image和key计算相似度，取topK个 key;image:(32,768) key:(260,768)   probability（32,260）batch,entityNum             @运算，矩阵相乘，等价于np.matmul(A, B)；t运算：转置
            ...
            # batch,classNum,ctx_dimension 32,10,768
            text_features = text_features.view(image_features.shape[0], (self.task_info['current_task']+1)*self.args.class_per_task, -1)
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts=None,getEmbedding=False):#text encoder,prompt.shape=torch.Size([107, 77, 768]),tokenized_prompt.shape=torch.Size([107, 77]),positional_embedding.shape=torch.Size([77, 768])

        # getEmbedding表示需要过token_embedding层
        if getEmbedding:
            ...
            x = x[torch.arange(x.shape[0]), (prompts if tokenized_prompts is None else tokenized_prompts).argmax(dim=-1)] @ self.text_projection
        return x
...
class PromptLearner(nn.Module):
    ...
    def forward(self, indexs , task_info , test_class = None, test = False):
        ...
        name_lens = allClass_tokenMap['name_lens']
        token_prefix = allClass_tokenMap['token_prefix']#100,1,768
        token_suffix = allClass_tokenMap['token_suffix']#100,16,768
        task_cls_num = len(test_class) if test else (current_task+1) * self.args.class_per_task
        tokenized_prompts = allClass_tokenMap['tokenized_prompts'][ : task_cls_num]#10,77
        ...
        for i in range( (current_task+1) * self.args.class_per_task ):
            ...
            prefix_i = token_prefix[i:i + 1, :, :].expand(batch, -1, -1)
            class_i = token_suffix[i:i + 1, :name_len, :].expand(batch, -1, -1)
            suffix_i = token_suffix[i:i + 1, name_len:, :].expand(batch, -1, -1)
            # ctx:(32,60,768);ctx_i:(32,1,60,768);prefix_i:(1,1,768);class_i:(1,1,768),suffix_i:(1,15,768)
        ...
        #320,154,768  task1:320,1,77,768->320,77,768
        prompts = prompts.view(batch * task_cls_num, -1, self.ctx_dim)
        #task1:10,77->10,77->320,77,           task2:20,77->10,154->320,154
        tokenized_prompts = tokenized_prompts.view(task_cls_num, -1)
        tokenized_prompts = tokenized_prompts.unsqueeze(0).expand(batch, -1, -1).contiguous().view(batch * task_cls_num, -1)
...
def load_and_build_clip(name: str,
